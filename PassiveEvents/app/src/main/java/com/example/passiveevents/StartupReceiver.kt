...
/**
 * Passive event subscriptions are not persisted across device restarts. This receiver checks if
 * we enabled passive events and, if so, registers them again.
 */
@AndroidEntryPoint
class StartupReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        runBlocking {
            if (repository.passiveEventsEnabled.first()) {
                ...
                if (result == PackageManager.PERMISSION_GRANTED) {
                    ...
                    // We may have lost the permission somehow. Mark that event subscriptions are
                    // disabled so the state is consistent the next time the user opens the app UI.
                    repository.setPassiveEventsEnabled(false)
                }
            }
        }
    }
    ...
    private fun scheduleWorker(context: Context) {
        ...
        // sometimes the call to subscribe for events takes longer than that and our
...
class SubscribeForEventsWorker @AssistedInject constructor(
    ...
    override fun doWork(): Result {
        ...
        runBlocking {
            healthServicesManager.subscribeForEvents()
        }
